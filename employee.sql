...
SET search_path TO user_schema;
----The Employee will not be able to execute the view due to not having permission, can only be executed by managers----
...
SELECT * FROM transactions;
SET search_path TO public;
----The employee will not be able to execute the function due to not having permission, can only be executed by managers----
SELECT approve_transaction(2);
Set search_path To user_schema;
SELECT * FROM transactions;
